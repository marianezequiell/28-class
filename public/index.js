...
socket.on('back', data => {
    ...
    if (data.length !== 0 && data.entities !== null || data.entities !== undefined) {
        
        let messages

        if (data.entities !== null && data.entities !== undefined) {
            messages = denormalize(data.result, [postSchema], data.entities)    
        } else {
            messages = data
        }
        ...
        for (let i = length - 1; i >= 0; i--) {
            ...
            p.innerHTML = `
                                <span class="chat_email">${messages[i].author.id} </span>
                                <span class="chat_hora">[${messages[i].text.hora}]: </span>
                                <span class="chat_text">${messages[i].text.message}</span>
                            `   
    ...
    if(document.getElementById('email').value === '') {
        ...
        const message = {
            ...
            author: {
                ...
                alias: document.querySelector('#nick').value,
                avatar: document.querySelector('#avatar').value,    
            },
            text: {id: undefined, message: document.querySelector('#message').value, hora: now}
        }
        socket.emit('notification', message)
    
        const emailProperty = document.querySelector('#email')
